...
Buffer* HeapVK::createBuffer(const BufferType type, const uint32 size)
{
...
Texture* HeapVK::createTexture(const TextureState state)
{
...
Heap* VulkanDevice::createHeap(const MemoryUsage usage, const uint32 size)
{
    ...
    for(uint32 i=0; i<memoryTypePerUsageCount[usageIndex]; ++i)
    {
        ...
        {
            return new HeapVK(std::dynamic_pointer_cast<VulkanDevice>(shared_from_this()),
                              handle,
