...
std::shared_ptr<Framebuffer> RenderPassD3D12::createFramebuffer(
    ...
    const TextureView* _depthResolve)
{
    ...
    if (depthStencil)
    {
        const TextureViewD3D12* view = _depthStencil ? reinterpret_cast<const TextureViewD3D12*>(_depthStencil)
                                                     : reinterpret_cast<const TextureViewD3D12*>(_stencil);
    ...
    const TextureView* _stencil)
{
    ...
    const TextureViewD3D12* view = reinterpret_cast<const TextureViewD3D12*>(swapChainSurface);
    assert( resolution.width  <= view->texture.state.width );
    assert( resolution.height <= view->texture.state.height );
    ...
    const TextureView* _stencil)
{
    ...
    // Resolution cannot be greater than selected destination size (Swap-Chain surfaces have no mipmaps)
    const TextureViewD3D12* source      = reinterpret_cast<const TextureViewD3D12*>(temporaryMSAA);
    const TextureViewD3D12* destination = reinterpret_cast<const TextureViewD3D12*>(swapChainSurface);
