...
// Optimisation: This table is not used. Backend type can be directly cast to D3D12 type by adding 1.
...
DepthStencilStateD3D12::DepthStencilStateD3D12(const DepthStencilStateInfo& desc)
{
    ...
    for(uint8 i=0; i<2; ++i)
    {
        ...
        stencil.StencilFailOp      = static_cast<D3D12_STENCIL_OP>(underlyingType(desc.stencil[i].whenStencilFails) + 1);  // Optimisation of: TranslateStencilOperation[desc.stencil[i].whenStencilFails];
        stencil.StencilDepthFailOp = static_cast<D3D12_STENCIL_OP>(underlyingType(desc.stencil[i].whenDepthFails) + 1);    // Optimisation of: TranslateStencilOperation[desc.stencil[i].whenDepthFails];
        stencil.StencilPassOp      = static_cast<D3D12_STENCIL_OP>(underlyingType(desc.stencil[i].whenBothPass) + 1);      // Optimisation of: TranslateStencilOperation[desc.stencil[i].whenBothPass];
        stencil.StencilFunc        = static_cast<D3D12_COMPARISON_FUNC>(underlyingType(desc.stencil[i].function) + 1);     // Optimisation of: TranslateCompareFunction[desc.stencil[i].function];
    }

    reference = static_cast<UINT>(desc.stencil[0].reference);
}

DepthStencilState* Direct3D12Device::createDepthStencilState(const DepthStencilStateInfo& desc)
{
