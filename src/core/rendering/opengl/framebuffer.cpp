...
//    bool Framebuffer::color(const DataAccess access, const uint8 index, const shared_ptr<Texture> texture, const uint16 layer, const uint8 mipmap)
//    {
//    // Default layer binding
//    Validate( glBindFramebuffer(accessType, id) )
//    if (type == Texture1D)
//       Validate( glFramebufferTexture1D(accessType, (GL_COLOR_ATTACHMENT0 + index), GL_TEXTURE_1D, tex->id, mipmap) )
//    if (type == Texture2D)
//       Validate( glFramebufferTexture2D(accessType, (GL_COLOR_ATTACHMENT0 + index), GL_TEXTURE_2D, tex->id, mipmap) )
//    // OpenGL 3.1
//    if (type == Texture2DRectangle)
//       Validate( glFramebufferTexture2D(accessType, (GL_COLOR_ATTACHMENT0 + index), GL_TEXTURE_RECTANGLE, tex->id, 0) )
//    // OpenGL 3.2
//    if (type == Texture2DMultisample)
//       Validate( glFramebufferTexture2D(accessType, (GL_COLOR_ATTACHMENT0 + index), GL_TEXTURE_2D_MULTISAMPLE, tex->id, 0) )
//
//    // Single layer binding
//    if (type == Texture3D)
//       {
//       assert( layer < texture->depth() );
//       Validate( glFramebufferTexture3D(accessType, (GL_COLOR_ATTACHMENT0 + index), GL_TEXTURE_3D, tex->id, mipmap, layer) )
//       }
...
//       Validate( glNamedFramebufferTexture(id, (GL_COLOR_ATTACHMENT0 + index), tex->id, mipmap) )
//    }
