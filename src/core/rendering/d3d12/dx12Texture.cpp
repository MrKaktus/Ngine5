...
Heap* TextureD3D12::parent(void) const
{
    return heap;
}

TextureView* TextureD3D12::view(void)
{
    return view(state.type, state.format, uint32v2(0, state.mipmaps), uint32v2(0, state.layers));
}

// TODO: If texture views are just pure descriptors, consider allocating them from the pool to avoid constant new/delete cycle
TextureView* TextureD3D12::view(const TextureType _type,
    const Format _format,
    const uint32v2 _mipmaps,
    const uint32v2 _layers)
{
    TextureViewD3D12* view = new TextureViewD3D12(*this,
                                                  _type,
                                                  _format,
                                                  _mipmaps,
                                                  _layers);
   
    //// Cache mipmaps and layers range, if View of other underlying type will be needed.
    //view->viewType = _type;
...
Texture* HeapD3D12::createTexture(const TextureState state)
{
    TextureD3D12* result = nullptr;
...
Texture& TextureViewD3D12::parent(void) const
{
    return texture;
}
...
ImageMemoryAlignment Direct3D12Device::textureMemoryAlignment(
