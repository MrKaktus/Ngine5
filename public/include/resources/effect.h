...
namespace en
{
   ...
   class Effect
         {
         private:
         std::string              shader[underlyingType(ShaderStage::Count)]; // Shaders code for each stage
         std::vector<std::string> code[underlyingType(ShaderStage::Count)];   // Preprocessor code autogenerated for each stage
         std::shared_ptr<gpu::Pipeline> binary;                      // Compiled and linked pipeline
         bool           dirty;                       // Do we need to recompile?
   
         public:
			 Effect(ShadingLanguage version, std::string name);
        ~Effect();
   
         void attach(ShaderStage stage, std::string code); // Add code to given shader stage
         ...
         std::shared_ptr<Pipeline> pipeline(void);                  // Return pipeline object
         };
