...
DescriptorSetD3D12::DescriptorSetD3D12(
        ...
        DescriptorsD3D12* _parent, 
        const uint64* _offsets, 
        const uint32* _slots, 
        RangeMapping* const _mappings, 
        uint32 _mappingsCount) :
    gpu(_gpu),
    parent(_parent),
    mappings(_mappings),
...
DescriptorSet* DescriptorsD3D12::allocate(const SetLayout& _layout)
{
    DescriptorSetD3D12* result = nullptr;

    const SetLayoutD3D12* layout = reinterpret_cast<const SetLayoutD3D12*>(&_layout);
    ...
    result = new DescriptorSetD3D12(gpu,
                                    this,
                                    &descriptorIndex[0],
...
bool DescriptorsD3D12::allocate(
    const uint32 count,
...
void CommandBufferD3D12::setDescriptors(
    ...
    const uint32 firstIndex)
{
    ...
    for(uint32 i=0; i<count; ++i)
    {
        const DescriptorSetD3D12* set = reinterpret_cast<const DescriptorSetD3D12*>(sets[i]);
    ...
    const ShaderStages stageMask)
{
    ...
    SetLayoutD3D12* result = new SetLayoutD3D12();
...
PipelineLayout* Direct3D12Device::createPipelineLayout(
    ...
    const ShaderStages stageMask)
{
    PipelineLayoutD3D12* result = nullptr; 
    ...
    if (immutableSamplersCount > 0)
    {
        ...
        for(uint32 i=0; i< immutableSamplersCount; ++i)
        {
            ...
            //
            samplers[i].Filter           = sampler->state.Filter;
            samplers[i].AddressU         = sampler->state.AddressU;
            samplers[i].AddressV         = sampler->state.AddressV;
            samplers[i].AddressW         = sampler->state.AddressW;
            samplers[i].MipLODBias       = sampler->state.MipLODBias;
            samplers[i].MaxAnisotropy    = sampler->state.MaxAnisotropy;
            samplers[i].ComparisonFunc   = sampler->state.ComparisonFunc;
            samplers[i].BorderColor      = sampler->border;
            samplers[i].MinLOD           = sampler->state.MinLOD;
            samplers[i].MaxLOD           = sampler->state.MaxLOD;
            samplers[i].ShaderRegister   = i; // All Immutable Samplers are enumerated in the order they are provided
            samplers[i].RegisterSpace    = setsCount; // Immutable Samplers always in last HLSL Space after all other Sets
  ...
  //D3D12_ROOT_PARAMETER1* tables = nullptr; // v1.1
    ...
    if (setsCount > 0)
    {
        ...
        if (setsCount > 1)
        {
            setBindingIndex = new uint32[setsCount];
            setBindingIndex[0] = 0;
        }
    ...
    {
        result = new PipelineLayoutD3D12(handle, setsCount, setBindingIndex);
    }
