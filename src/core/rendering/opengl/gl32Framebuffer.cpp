...
namespace en
{
   ...
   void FramebufferGL32::color(const DataAccess access, const uint8 index, const Ptr<Texture> texture, const uint8 mipmap)
   {
 ...
 //assert( type != TextureBuffer );
   ...
   void FramebufferGL32::color(const DataAccess access, const uint8 index, const Ptr<Texture> texture, const uint16 layer, const uint8 mipmap)
   {
   // API independent debug validation layer
#ifdef EN_VALIDATE_GRAPHIC_CAPS_AT_RUNTIME
   assert( texture );
