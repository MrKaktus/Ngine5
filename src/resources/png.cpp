...
#define PageSize 4096
   ...
   // Determine destination GPU to which texture will be uploaded
   Ptr<Heap> heap = dst->parent();
   Ptr<GpuDevice> gpu = heap->device();

   // Alloc buffers for IDAT chunks data and decompressed stream
   uint32 size     = (settings.width * settings.height * gpu->texelSize(settings.format));
   ...
   uint32 texelSize  = gpu->texelSize(settings.format);
   ...
   Ptr<gpu::CommandBuffer> command = gpu->createCommandBuffer(type);
   command->start();
   ...
   // Determine destination GPU to which texture will be uploaded
   Ptr<Heap> heap = nullptr; //dst->parent(); <----------------------------- TODO: FIXME!!!
   Ptr<GpuDevice> gpu = en::Graphics->primaryDevice(); //heap->device();
   ...
   Ptr<gpu::CommandBuffer> command = gpu->createCommandBuffer(type);
   command->start();
   command->barrier(texture, uint32v2(0, 1), uint32v2(0, 1), static_cast<TextureAccess>(0), TextureAccess::CopyDestination); 
