...
// Optimisation: This table is not needed. Frontend type can be directly cast to D3D12 type by adding 1.
...
Sampler* Direct3D12Device::createSampler(const SamplerState& state)
{
    ...
    return new SamplerD3D12(samplerInfo, TranslateStaticSamplerBorder[underlyingType(state.borderColor)]);
};
