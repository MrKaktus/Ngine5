...
namespace en
{
   ...
   class Effect
         {
         private:
         string         shader[underlyingType(PipelineStage::Count)]; // Shaders code for each stage
         vector<string> code[underlyingType(PipelineStage::Count)];   // Preprocessor code autogenerated for each stage
         Ptr<Pipeline>  binary;                      // Compiled and linked pipeline
        ...
        ~Effect();
         ...
         Ptr<Pipeline> pipeline(void);                  // Return pipeline object
         };
